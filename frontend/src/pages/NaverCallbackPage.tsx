...
export function NaverCallbackPage() {
  ...
  const { handleNaverCallback, error } = useAuthStore();
  ...
  useEffect(() => {
    const processCallback = async () => {
      ...
      logger.log('[NaverCallback] Processing callback...', { code: !!code, state: !!state, error: errorParam });

      // 에러 응답 처리
      if (errorParam) {
        logger.error('[NaverCallback] OAuth error:', errorParam, errorDescription);
      ...
      // 필수 파라미터 확인
      if (!code || !state) {
        logger.error('[NaverCallback] Missing code or state');
        setIsProcessing(false);
        navigate('/login', {
    ...
    processCallback();
  }, [searchParams, handleNaverCallback, navigate]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="text-center">
        {isProcessing ? (
          <>
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#03C75A] mx-auto mb-4" />
