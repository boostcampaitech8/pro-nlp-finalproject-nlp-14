...
| `mit_merge` | 합의된 내용을 GT로 확정 | UC-007 | 정의됨 |
| `mit_summary` | 현재까지 회의 내용 요약 | UC-012 | 정의됨 |
| `mit_action` | Action Item 추출 및 정리 | UC-013 | 정의됨 |
...
| Tool | 설명 | 유즈케이스 | 상태 |
|------|------|-----------|------|
| `mcp_jira` | Jira 이슈 생성/조회/수정 | UC-015 | 정의됨 |
| `mcp_notion` | Notion 페이지 생성/수정 | UC-016 | 정의됨 |
| `mcp_slack` | Slack 메시지 전송 | UC-017 | 정의됨 |
| `mcp_calendar` | 일정 조회/생성 | UC-018 | 정의됨 |
| `mcp_drive` | 문서 저장/공유/검색 | UC-019 | 정의됨 |
