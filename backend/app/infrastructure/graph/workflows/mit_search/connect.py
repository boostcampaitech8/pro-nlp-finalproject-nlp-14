...
def build_mit_search() -> StateGraph:
    ...
    그래프 구조 (병렬 Speculative RAG):
        START → query_rewriter → query_intent_analyzer →
                    ↓
            [병렬 분기]
            ├─ cypher_generator → tool_executor
            └─ vector_search_async
                    ↓
            result_merger → tool_result_scorer → reranker → selector → END
    ...
    # 선형 부분: START → query_rewriter → query_intent_analyzer
    workflow.add_edge(START, "query_rewriter")
    workflow.add_edge("query_rewriter", "query_intent_analyzer")
    ...
    # 병합 후 순차 처리
    workflow.add_edge("result_merger", "tool_result_scorer")
