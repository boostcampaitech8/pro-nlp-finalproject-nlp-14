...
class GeneratePrState(TypedDict, total=False):
    ...
    generate_pr_transcript_utterances: Annotated[
        list[dict],
        "회의 발화 목록 [{id, speaker_name, text, start_ms, end_ms}]",
    ]
    generate_pr_realtime_topics: Annotated[
        list[dict],
        "실시간 L1 토픽 스냅샷 (API enqueue 시점 전달)",
    ]

    # 라우팅/청킹 상태
    generate_pr_route: Annotated[str, "short|long 라우팅 결과"]
    generate_pr_chunks: Annotated[list[dict], "청킹된 추출 단위 목록"]
    ...
    #     "topic": "아젠다 주제",
    #     "description": "설명",
    #     "decision": [{"content": "결정 내용", "context": "맥락"}]
    # }]
