...
사용 예시:
    ...
    app = await get_voice_orchestration_app(with_checkpointer=True)
    config = {"configurable": {"thread_id": meeting_id}}
    result = await app.ainvoke(state, config)

    # checkpointer 없이 (단일 턴)
    app = await get_voice_orchestration_app(with_checkpointer=False)
...
def route_after_simple_check(state: VoiceOrchestrationState) -> str:
    """간단한 쿼리 여부에 따라 라우팅"""
...
async def get_voice_orchestration_app(
    *,
    ...
    """
    workflow = build_voice_orchestration_workflow()
