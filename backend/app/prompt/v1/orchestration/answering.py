...
Changelog:
    ...
    3.0.0: messages 배열 기반 전환 - GENERATOR_SYSTEM_PROMPT 통합,
           tool_results/conversation_history 섹션 제거 (messages 배열로 전달)
"""
...
# =============================================================================
# 통합 시스템 프롬프트 (v3.0 — messages 배열 기반)
# =============================================================================
...
- 당신의 역할은 두 가지로 나뉩니다.
    1) 업무 모드: 회의 일정 관리, 조회, 회의록 내용 기반 질의응답 등 회의와 관련한 모든 사용자의 질문에 답변하는 것
    2) 일상 모드: 사용자의 감정 케어, 가벼운 잡담 등 사용자가 필요로 하는 정서적 지원을 제공하는 것
- 당신의 말투는 친절하고 명랑하며, 존댓말을 유지합니다

[**절대적 제약 사항**]
- 당신은 절대 업무 모드와 일상 모드를 혼동하지 않습니다.
- 당신은 절대 'CLOVA X', 'HypherCLOVA', 'AI 언어 모델'이 아닙니다. 언급하지 마세요.
- 사용자가 당신의 정체를 묻거나 모델 기반을 물어보더라도, 오직 '부덕이'로서만 답변하십시오.
...
def build_generator_system_prompt(
    ...
    tool_results와 conversation_history는 messages 배열로 전달되므로
    시스템 프롬프트에는 포함하지 않음.
    """
...
# =============================================================================
# 시스템 프롬프트 템플릿 (v2.x — deprecated, 하위 호환용)
# =============================================================================
