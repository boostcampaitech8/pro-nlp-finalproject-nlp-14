...
Changelog:
    ...
    3.0.3: agenda description 문체 고정 — 서술형 문장 대신 명사구(체언형)로 출력
    3.0.4: decision 유연성 강화 — 방향성/원칙 합의 및 약한 합의 표현도 decision으로 포착
...
4. 각 아젠다에서 **실행 방침/방향/원칙이 합의된 경우** decision을 추출하세요.
   - 명시적 합의/확정/결정 표현이 있으면 우선 decision으로 추출하세요.
   - 명시어가 약해도, 팀이 실제 실행안을 하나로 정한 발화(예: "~로 진행합시다", "~로 가죠", "~로 하자", "~로 정리합시다", "우선 ~ 기준으로 가겠습니다")는 decision으로 볼 수 있습니다.
   - 잠정 확정(현재 시점 기준의 실행 기준/우선안 합의)도 decision으로 포함하세요.
   - decision이 없으면 null로 두세요.
   - 액션 아이템(TODO), 단순 의견, 대안만 나열된 탐색 대화, 채택 여부가 미확정인 제안/아이디어는 decision이 아닙니다.
   - 팀의 진행 방향/원칙 또한 decision으로 포함하세요.
...
3. **description** (보충 설명): 근거 원문을 바탕으로 논의의 맥락이나 배경을 **명사구(체언형) 1개**로 작성하세요.
   - 근거 원문에 있는 내용만 사용
   - 근거 원문 문장을 그대로 복사하지 말고, 의미를 유지해 재서술하세요
   - **서술형 종결 표현 금지**: "했다", "하였다", "논의했다", "공유했다", "확인했다", "결정했다" 등 문장형 종결어미를 쓰지 마세요
   - **명사구 스타일 유지**: "~공유", "~논의", "~검토", "~합의", "~확정", "~조율", "~정리" 형태로 작성하세요
   - 예: "다양한 모델의 성능 평가 결과를 공유하고 개선 방안을 논의했다." → "다양한 모델의 성능 평가 결과 공유 및 개선 방안 논의"
   - 화자명 접두어(예: "김민준:")나 직접 인용 형태는 사용하지 마세요
   - 근거 원문과 7어절 이상 연속으로 동일한 표현을 사용하지 마세요
   - 길이는 12~40자 내외로 간결하게 유지하세요
   - 추측이나 일반 지식을 추가하지 마세요
   - self-check: description이 문장형 종결("다/했다/하였다")로 끝나면 반드시 다시 작성하세요
   - self-check: description이 근거 원문의 부분 문자열이면 반드시 다시 작성하세요

### Decision 작성
4. decision 키워드가 있는 경우에만 decision을 작성하세요.
5. **content**: 다음 형식으로 작성하세요:
   - `[누가(선택)] [무엇을(필수)] [언제까지(선택)] [결정/합의/확정/진행하기로/정리]`
   - 예: "리프레시 토큰 만료 시 재로그인 유도 문구를 표시하기로 결정"
   - 예: "2차 배포를 3월 31일로 확정"
   - 예: "김민준이 Redis 캐시 도입 방안을 다음 주까지 문서화하기로 합의"
   - 예: "우선 Redis 캐시 도입 기준으로 진행"
   - 약한 합의 표현이라도 Step 1에서 decision으로 판단된 경우 임의로 null로 바꾸지 마세요.
